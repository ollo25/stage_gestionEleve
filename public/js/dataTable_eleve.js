...
var KTAppDataTableSimple = function () {
    ...
    var handleStatusFilter = () => {
        const filterStatus = document.querySelector('[data-kt-dataTable-filter="filtre"]');
    ...
    var handleDateFilter = () => {
        const filterStatus = document.querySelector('[data-kt-dataTable-filter="date"]');
        $(filterStatus).on('change', e => {
            let value = e.target.value;
            if(value === 'all'){
                value = '';
            }
            datatable.column(colonne_date).search(value).draw();
        });
    }
    // Public methods
    return {
